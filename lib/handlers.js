...
import { createNextTimeElement, getDirection, promoteNextNumberToCurrent } from "./time-animation.js";
...
export const handleTransitionToggle = () => {
  const incomingElements = Array.from(document.querySelectorAll('.incoming-number:not(.changing)'));
  incomingElements.forEach(incoming => {
    const clonedIncoming = createNextTimeElement();
    const timeElement = incoming.closest('.time-number');
    timeElement.replaceChild(clonedIncoming, incoming);
